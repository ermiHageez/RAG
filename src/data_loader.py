...
def load_all_documents(data_dir: str) -> List[Any]:
    ...
    for txt_file in text_files:
        ...
        try:
            ...
            loaded = loader.load()
            print(
                f"[DEBUG] Found {len(text_files)} TXT files: {[str(f) for f in text_files]}"
            )
            documents.extend(loaded)
        except Exception as e:
    ...
    print(f"[DEBUG] EXCEL files :{len(xlsx_files)} | EXCEL files found : {xlsx_files}")
    for xlsx_file in xlsx_files:
        print("[DEBUG] LOADING EXCEL {xlsx_file}")
        try:
